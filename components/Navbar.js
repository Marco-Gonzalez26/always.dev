          ...
          {links?.map(({ title, path }) => {
            return (
              <Link key={path} href={path}>
                <a className="hover:scale-105 hover:opacity-75 transition-all text-lg">
                  {title}
                </a>
              </Link>
            )
      ...
      {open && (
        <AnimatePresence>
          <motion.ul
            ...
            animate={{ x: 0 }}
            key="motion.ul"
            className=" flex flex-col gap-10 fixed top-0 right-0  h-full lg:w-1/3  w-1/2 items-center p-5  lg:text-3xl sm:text-xl drop-shadow-lg bg-white border-l border-black md:hidden z-50 text-lg"
          >
